interface DataItem {
  ...
  function HandleInputData({
    ...
    const symmetricalPoint_Y = cy;
      const path_2 = `M ${startX} ${startY} 
          A ${radius} ${radius} 0 0 0 ${symmetricalPoint_X} ${symmetricalPoint_Y}
          A ${radius} ${radius} 0 0 0 ${endpoint_Arc_X} ${endpoint_Arc_Y}
          `;
    return path_2;
  }
    ...
    data.forEach((item, i) => {
      if (i === 0) {
        if (item.percent > 50) {
          ...
          const path = `M ${cx + radius} ${cy} A ${radius} ${radius} 0 0 0 ${x1} ${y1}`;
          pathLists.push({
            x: x1,
            y: y1,
            path: path,
        ...
        const path = `M ${x1} ${y1} A ${radius} ${radius} 0 0 0 ${x_Next} ${y_Next}`;
        pathLists.push({
          x: x_Next,
          y: y_Next,
          path: path,
